...
from keras_detection.heads import SingleConvHead, SingleConvPoolHead
...
class HeadsTest(tf.test.TestCase):
    ...
    def test_single_conv_pool_head(self):

        head = SingleConvPoolHead(num_outputs=4, output_name="boxes")
        inputs = utils.create_fake_input_map(1, 32, 24, head.num_outputs)
        outputs = head.forward(inputs, quantized=False)
        self.assertEqual(outputs.shape.as_list(), [1, 4])
        q_outputs = head.forward(inputs, quantized=True)
        self.assertEqual(q_outputs.shape.as_list(), [1, 4])
        head._head_model.summary()